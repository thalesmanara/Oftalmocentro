...
const format = (norm.query || {}).format || 'json';
...
return [{ json: {
  data: run ? { run, metrics, results, format } : { error: 'RUN_NOT_FOUND' },
  asList: false,
  statusCode,
...
const webhook = trigger({
  ...
  config: {
    name: 'Webhook',
    parameters: { path: 'system/ai-eval/export', responseMode: 'responseNode', options: {} },
  },
});
...
export default workflow('get-system-ai-eval-export', 'GET System AI Eval Export')
  .add(webhook)
  .to(normalizar)
  .to(validarAuth)
