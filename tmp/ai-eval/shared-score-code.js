/** Shared deterministic scoring + helpers for n8n Code nodes (no OpenAI). */
...
function scoreCase(input) {
  ...
  const refused =
    includesWord(answer, refusalPhrase) ||
    normalizeText(answer) === normalizeText(refusalPhrase);
  let requiredHit = 0;
  ...
  const hasExpectedDoc = expectedDocId ? sourceIds.includes(String(expectedDocId)) : true;
  ...
  let sourcesScore = 0;
  if (expectNoAnswer) sourcesScore = refused ? 30 : 0;
  else if (requiredSourceId) sourcesScore = hasRequiredSource ? 30 : 0;
  else sourcesScore = sourceIds.length > 0 ? 25 : 10;
  let documentScore = 0;
  if (expectNoAnswer) documentScore = refused ? 20 : 0;
  else if (expectedDocId) documentScore = hasExpectedDoc ? 20 : 0;
  else documentScore = 15;
  let latencyScore = 0;
  ...
  let score = Math.round(
    Math.max(0, Math.min(100, answerQuality + sourcesScore + documentScore + latencyScore)),
  );
...
function aggregateMetrics(results) {
  ...
  const recall = answerable.length
    ? Math.round(
        (answerable.filter((r) => r.verdict === 'PASS').length / answerable.length) * 10000,
      ) / 100
    : 0;
  ...
  const docsHit = new Set(
    results
      .filter((r) => r.matchedDocument && r.expectedDocumentId)
      .map((r) => String(r.expectedDocumentId)),
  );
  ...
  for (const r of results) {
    for (const s of r.sources || []) {
      ...
      if (!docCounts[id])
        docCounts[id] = {
          documentId: id,
          title: s.documentTitle || s.document_title,
          count: 0,
        };
...
module.exports = { scoreCase, aggregateMetrics, normalizeText };
