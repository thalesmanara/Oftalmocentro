...
const verdict = first ? first.verdict : 'ERROR';
const score = first ? first.score : null;
const caseCode = first ? first.caseCode : ($('Webhook').first().json.body || {}).caseCode || null;
const testAuditAction = verdict === 'PASS' ? 'TEST_EXECUTED' : 'TEST_FAILED';
let userId = '';
let sessionId = '';
...
const webhook = trigger({
  ...
  config: {
    name: 'Webhook',
    parameters: { path: 'system/ai-eval/run-case', httpMethod: 'POST', responseMode: 'responseNode', options: {} },
  },
});
    ...
    parameters: {
      ...
      workflowInputs: {
        ...
        value: {
          ...
          metadata: "={{ { caseId: ($('Webhook').first().json.body || {}).caseId || null, caseCode: ($('Webhook').first().json.body || {}).caseCode || null, triggerMode: 'single' } }}",
        },
      },
    },
  },
    ...
    parameters: {
      ...
      workflowInputs: {
        ...
        value: {
          ...
          caseId: "={{ ($('Webhook').first().json.body || {}).caseId || '' }}",
          caseCode: "={{ ($('Webhook').first().json.body || {}).caseCode || '' }}",
          triggerMode: 'single',
        },
      },
    },
  ...
  config: {
    ...
    parameters: {
      ...
      workflowInputs: {
        mappingMode: 'defineBelow',
        value: {
          action: "={{ $('Montar resposta admin').first().json.testAuditAction }}",
          resourceType: 'ai_test_run',
          resourceId: "={{ $('Montar resposta admin').first().json.runId || '' }}",
          success: "={{ $('Montar resposta admin').first().json.testAuditAction === 'TEST_EXECUTED' }}",
          requestId: '={{ $json.requestId }}',
          ...
          sessionId: "={{ $('Validar auth').first().json.sessionId || '' }}",
          metadata:
            "={{ { runId: $('Montar resposta admin').first().json.runId, caseCode: $('Montar resposta admin').first().json.testCaseCode, verdict: $('Montar resposta admin').first().json.testVerdict, score: $('Montar resposta admin').first().json.testScore } }}",
        },
      },
    },
  },
...
export default workflow('post-system-ai-eval-run-case', 'POST System AI Eval Run Case')
  .add(webhook)
  .to(normalizar)
  .to(validarAuth)
