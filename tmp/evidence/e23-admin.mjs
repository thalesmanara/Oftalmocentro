#!/usr/bin/env node
/**
 * Etapa 23 — admin evidence endpoints cloned from SYSTEM - AI CONTEXT LIST pattern.
 */
import pg from 'pg';
import { randomUUID, createHash } from 'crypto';
import { writeFileSync } from 'fs';
import { defaultEvidenceConfig, validateEvidenceConfiguration } from './evidence-helpers.mjs';
...
async function upsertWorkflow({ id, name, nodes, connections, active = true }) {
  ...
  await client.query(
    `INSERT INTO workflow_history ("versionId","workflowId",authors,nodes,connections,name,description,autosaved,"createdAt","updatedAt")
     VALUES ($1::varchar,$2,'etapa23',$3::json,$4::json,$5,'Etapa 23 admin',false,NOW(),NOW())`,
...
const specs = [
  {
    id: 'c23EvidenceList0001',
    name: 'SYSTEM - AI EVIDENCE LIST',
    path: 'system/ai-evidence',
    method: 'GET',
...
const sql=\`SELECT jsonb_build_object(
  'items', jsonb_build_array(jsonb_build_object(
    ...
    'activeMode', (SELECT value FROM app_secrets WHERE key='evidence_active_mode' LIMIT 1),
    'activeVersionLabel', (SELECT value FROM app_secrets WHERE key='evidence_active_version' LIMIT 1),
    ...
    'draftCount', (SELECT COUNT(*)::int FROM ai_evidence_config_versions WHERE status='DRAFT'),
    'versionCount', (SELECT COUNT(*)::int FROM ai_evidence_config_versions)
  ))
) AS data
FROM ai_evidence_configs d WHERE d.code='AI_QUERY_EVIDENCE' LIMIT 1\`;
...
return [{json:{data:data||{items:[]},statusCode:200,auditAction:'AI_EVIDENCE_COMPLETED'}}];`,
  },
  {
    id: 'c23EvidenceDetail001',
    name: 'SYSTEM - AI EVIDENCE DETAIL',
    path: 'system/ai-evidence/detail',
    method: 'GET',
    restoreCode: `const auth=$('Validar auth').first().json||{};
...
const sql=\`SELECT jsonb_build_object(
  ...
  'version', CASE WHEN '\${versionId}'<>'' THEN (
    SELECT jsonb_build_object(
      'id',v.id,'versionNumber',v.version_number,'versionLabel',v.version_label,'status',v.status,'mode',v.mode,
      'configuration',v.configuration,'contentHash',v.content_hash,'notes',v.notes,'createdAt',v.created_at,'publishedAt',v.published_at
    ) FROM ai_evidence_config_versions v WHERE v.id='\${versionId}'::uuid LIMIT 1
  ) ELSE NULL END
) AS data\`;
return [{json:{requestId:auth.requestId||'',userId:auth.userId,sessionId:auth.sessionId,requestStartedAtMs:Date.now(),sql}}];`,
    montarCode: `const row=$input.first().json||{};
let data=row.data; if(typeof data==='string'){try{data=JSON.parse(data);}catch(_){data={};}}
return [{json:{data:data||{},statusCode:200,auditAction:'AI_EVIDENCE_COMPLETED'}}];`,
  },
  {
    id: 'c23EvidenceCompare01',
    name: 'SYSTEM - AI EVIDENCE COMPARE',
    path: 'system/ai-evidence/compare',
    method: 'GET',
    restoreCode: `const auth=$('Validar auth').first().json||{};
const sql=\`SELECT jsonb_build_object(
  'published', (SELECT to_jsonb(v) FROM ai_evidence_config_versions v WHERE status='PUBLISHED' ORDER BY published_at DESC NULLS LAST LIMIT 1),
  'drafts', COALESCE((SELECT jsonb_agg(to_jsonb(v) ORDER BY version_number DESC) FROM ai_evidence_config_versions v WHERE status='DRAFT'),'[]'::jsonb),
  'secrets', jsonb_build_object(
    'mode', (SELECT value FROM app_secrets WHERE key='evidence_active_mode' LIMIT 1),
    'version', (SELECT value FROM app_secrets WHERE key='evidence_active_version' LIMIT 1)
  )
) AS data\`;
return [{json:{requestId:auth.requestId||'',userId:auth.userId,sessionId:auth.sessionId,requestStartedAtMs:Date.now(),sql}}];`,
    montarCode: `const row=$input.first().json||{};
...
// Mutating endpoints: create/update/validate/publish/rollback — compact custom WFs
...
const mutating = [
  makeMutating({
    id: 'c23EvidenceCreate001',
    name: 'SYSTEM - AI EVIDENCE CREATE',
    path: 'system/ai-evidence/create',
    method: 'POST',
    restoreCode: `const auth=$('Validar auth').first().json||{};
const body=$('Webhook').first().json.body||{};
const mode=String(body.mode||'STRUCTURED').toUpperCase();
const label=String(body.versionLabel||('evidence-draft-'+Date.now())).replace(/'/g,"''").slice(0,80);
...
const sql=\`WITH def AS (SELECT id FROM ai_evidence_configs WHERE code='AI_QUERY_EVIDENCE' LIMIT 1),
n AS (SELECT COALESCE(MAX(version_number),0)+1 AS vn FROM ai_evidence_config_versions WHERE evidence_config_id=(SELECT id FROM def)),
ins AS (
  INSERT INTO ai_evidence_config_versions (evidence_config_id, version_number, version_label, status, mode, configuration, content_hash, notes)
...
return [{json:{requestId:auth.requestId||'',userId:auth.userId,sessionId:auth.sessionId,requestStartedAtMs:Date.now(),sql}}];`,
  ...
  makeMutating({
    id: 'c23EvidenceUpdate001',
    name: 'SYSTEM - AI EVIDENCE UPDATE',
    path: 'system/ai-evidence/update',
    method: 'PUT',
    restoreCode: `const auth=$('Validar auth').first().json||{};
const body=$('Webhook').first().json.body||{};
...
const hash=require('crypto').createHash('sha256').update(JSON.stringify(cfg)).digest('hex');
const sql=\`WITH u AS (
  UPDATE ai_evidence_config_versions SET mode='\${mode}', version_label=COALESCE(NULLIF('\${label}',''),version_label),
...
return [{json:{requestId:auth.requestId||'',userId:auth.userId,sessionId:auth.sessionId,requestStartedAtMs:Date.now(),sql}}];`,
  ...
  makeMutating({
    id: 'c23EvidenceValidate01',
    name: 'SYSTEM - AI EVIDENCE VALIDATE',
    path: 'system/ai-evidence/validate',
    method: 'POST',
    restoreCode: `const auth=$('Validar auth').first().json||{};
const body=$('Webhook').first().json.body||{};
...
for(const b of ['enableEvidenceScore','enableClassification','enableConflictConsolidation','enableRedundancyDetection','enableRichSources','passthroughToCwm','dropBelowMinScore']){
  if(cfg[b]!==undefined && typeof cfg[b]!=='boolean') errors.push({field:b,message:'deve ser boolean'});
}
if(cfg.minEvidenceScore!==undefined && (typeof cfg.minEvidenceScore==='string'||!Number.isFinite(Number(cfg.minEvidenceScore)))) errors.push({field:'minEvidenceScore',message:'número 0-100'});
const ok=errors.length===0;
const sql="SELECT jsonb_build_object('ok',"+ok+",'errors','"+JSON.stringify(errors).replace(/'/g,"''")+"'::jsonb,'configuration','"+JSON.stringify(cfg).replace(/'/g,"''")+"'::jsonb) AS data";
return [{json:{requestId:auth.requestId||'',userId:auth.userId,sessionId:auth.sessionId,requestStartedAtMs:Date.now(),sql,statusCode:ok?200:400}}];`,
  ...
  makeMutating({
    id: 'c23EvidencePublish01',
    name: 'SYSTEM - AI EVIDENCE PUBLISH',
    path: 'system/ai-evidence/publish',
    method: 'POST',
    restoreCode: `const auth=$('Validar auth').first().json||{};
const body=$('Webhook').first().json.body||{};
const versionId=String(body.versionId||'').replace(/[^0-9a-f-]/gi,'');
const sql=\`WITH arch AS (
  ...
  UPDATE ai_evidence_config_versions SET status='PUBLISHED', published_at=NOW()
  WHERE id='\${versionId}'::uuid RETURNING id, version_label, mode
), sec AS (
  UPDATE app_secrets SET value=(SELECT mode FROM pub), updated_at=NOW() WHERE key='evidence_active_mode'
), sec2 AS (
  UPDATE app_secrets SET value=(SELECT version_label FROM pub), updated_at=NOW() WHERE key='evidence_active_version'
)
SELECT jsonb_build_object('ok',true,'published',to_jsonb(pub),'archived',(SELECT COUNT(*) FROM arch)) AS data FROM pub\`;
return [{json:{requestId:auth.requestId||'',userId:auth.userId,sessionId:auth.sessionId,requestStartedAtMs:Date.now(),sql}}];`,
  }),
  makeMutating({
    id: 'c23EvidenceRollback1',
    name: 'SYSTEM - AI EVIDENCE ROLLBACK',
    path: 'system/ai-evidence/rollback',
    method: 'POST',
    restoreCode: `const auth=$('Validar auth').first().json||{};
const body=$('Webhook').first().json.body||{};
const target=String(body.targetVersionId||body.versionId||'').replace(/[^0-9a-f-]/gi,'');
const sql=\`WITH cur AS (
  SELECT id FROM ai_evidence_config_versions WHERE status='PUBLISHED' LIMIT 1
), arch AS (
  UPDATE ai_evidence_config_versions SET status='ARCHIVED', archived_at=NOW()
...
return [{json:{data:data||{},statusCode:prep.statusCode||200,auditAction:'AI_EVIDENCE_COMPLETED'}}];`;
  const vid = await upsertWorkflow({ id: spec.id, name: spec.name, nodes, connections, active: true });
  await registerWebhook(spec.id, spec.method, spec.path, nodes);
  created[spec.id] = vid;
  console.log('OK', spec.name);
...
// Health patch — add evidenceLayer
...
if (probe && !String(probe.parameters.query).includes('evidence_stats')) {
  probe.parameters.query = String(probe.parameters.query).replace(
    'retrieval_stats AS (',
    `evidence_stats AS (
  SELECT
    COALESCE((SELECT value FROM app_secrets WHERE key='evidence_active_mode' LIMIT 1),'STRUCTURED') AS evidence_mode,
    COALESCE((SELECT value FROM app_secrets WHERE key='evidence_active_version' LIMIT 1),'evidence-v1') AS evidence_version,
    (SELECT COUNT(*)::int FROM ai_evidence_config_versions WHERE status='DRAFT') AS evidence_drafts,
    (SELECT COUNT(*)::int FROM ai_evidence_config_versions WHERE status='PUBLISHED') AS evidence_published
),
retrieval_stats AS (`,
  );
  probe.parameters.query = String(probe.parameters.query).replace(
    ...
    `retrieval_stats.retrieval_last_validation,
  evidence_stats.evidence_mode,
  evidence_stats.evidence_version,
  evidence_stats.evidence_drafts,
  evidence_stats.evidence_published,`,
  );
...
if (prep && !prep.parameters.jsCode.includes('evidenceDb')) {
  prep.parameters.jsCode = prep.parameters.jsCode.replace(
    'const cacheDb = {',
    `const evidenceDb = {
      mode: dbItem.evidence_mode || 'STRUCTURED',
      version: dbItem.evidence_version || 'evidence-v1',
      drafts: Number(dbItem.evidence_drafts ?? 0) || 0,
      published: Number(dbItem.evidence_published ?? 0) || 0,
      available: true,
    };
const cacheDb = {`,
  );
...
if (agg && !agg.parameters.jsCode.includes('evidenceLayer:')) {
  agg.parameters.jsCode = agg.parameters.jsCode.replace(
    'semanticCache: (() => {',
    `evidenceLayer: (() => {
    const e = partial.evidenceDb || {};
    const published = Number(e.published || 0);
    let status = 'ok';
    if (published !== 1) status = 'degraded';
    return {
      status,
      activeMode: e.mode || 'STRUCTURED',
      activeVersion: e.version || 'evidence-v1',
      averageEvidenceScore: null,
      conflictRate: null,
      redundancyRate: null,
      evidencesPerQuery: null,
      ocrDistribution: null,
      tabularDistribution: null,
...
if (montarAdmin && !montarAdmin.parameters.jsCode.includes("'evidenceLayer'")) {
  montarAdmin.parameters.jsCode = montarAdmin.parameters.jsCode.replace(
    "'semanticCache']",
    "'semanticCache','evidenceLayer']",
  );
  if (!montarAdmin.parameters.jsCode.includes("key === 'evidenceLayer'")) {
    montarAdmin.parameters.jsCode = montarAdmin.parameters.jsCode.replace(
      "if (key === 'semanticCache') {",
      `if (key === 'evidenceLayer') {
    out.activeMode = c.activeMode || null;
    out.activeVersion = c.activeVersion || null;
    out.averageEvidenceScore = c.averageEvidenceScore != null ? Number(c.averageEvidenceScore) : null;
    out.conflictRate = c.conflictRate != null ? Number(c.conflictRate) : null;
    out.redundancyRate = c.redundancyRate != null ? Number(c.redundancyRate) : null;
...
for (const [id, nodes] of [
  ...
  await client.query(
    `INSERT INTO workflow_history ("versionId","workflowId",authors,nodes,connections,name,description,autosaved,"createdAt","updatedAt")
     VALUES ($1::varchar,$2,'etapa23',$3::json,$4::json,$5,'evidence health',false,NOW(),NOW())`,
...
// Backup workflow mention — append note into BACKUP if exists
...
for (const b of backup.rows) {
  ...
  if (code && !code.parameters.jsCode.includes('ai_evidence_configs')) {
    code.parameters.jsCode = code.parameters.jsCode.replace(
      'ai_cache_configs',
      'ai_cache_configs, ai_evidence_configs, ai_evidence_config_versions',
    );
    const versionId = randomUUID();
    await client.query('BEGIN');
    await client.query(
      `INSERT INTO workflow_history ("versionId","workflowId",authors,nodes,connections,name,description,autosaved,"createdAt","updatedAt")
       SELECT $1::varchar, id, 'etapa23', $2::json, connections, name, 'include evidence tables', false, NOW(), NOW() FROM workflow_entity WHERE id=$3`,
...
writeFileSync(new URL('./_e23-admin.json', import.meta.url), JSON.stringify(created, null, 2));
await client.end();
