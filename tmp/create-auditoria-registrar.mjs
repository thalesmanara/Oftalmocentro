...
const sanitizeJs = fs.readFileSync(
  new URL('./auditoria-registrar-code.js', import.meta.url),
  'utf8'
)

const finalizeJs = `
const prep = $('Sanitizar').first();
...
export default workflow('auditoria-registrar', 'AUDITORIA - REGISTRAR')
  ...
  .to(
    shouldInsert
      .onTrue(insert.to(finalizeOk))
      .onFalse(finalizeSkip)
  )
