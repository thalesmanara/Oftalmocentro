#!/usr/bin/env node
/**
 * Etapa 22.1 — clone admin auth from SYSTEM - AI CACHE LIST for metrics/entries/shadow endpoints.
 */
import pg from 'pg';
...
async function registerWebhook(wfId, method, path, nodes) {
  ...
  await client.query(
    ...
    [wfId, path, method, nodeName, String(webhookId), pathLength],
  );
}
...
function cloneAdmin({ id, name, path, method, sqlCode, outCode }) {
  ...
  // Find SQL / montar nodes in list — replace their code
  const sqlNode =
    nodes.find((n) => /montar sql|sql|query|carregar/i.test(n.name) && n.type.includes('code')) ||
    nodes.find((n) => n.type.includes('postgres'));
  const pgNode = nodes.find((n) => n.type.includes('postgres'));
  const outNode =
    nodes.find((n) => /montar resposta|montar result|responder/i.test(n.name) && n.type.includes('code')) ||
    nodes.find((n) => n.name === 'Montar lista' || n.name === 'Montar');

  // Dump names for first create
  return { nodes, connections, wh, sqlNode, pgNode, outNode, sqlCode, outCode, id, name, path, method };
}
...
const specs = [
  {
    ...
    method: 'GET',
    sqlCode: `const q=($('Restaurar request').first()?.json?.query)||($('Webhook').first()?.json?.query)||{};
...
return [{json:{sql, statusCode:200, finishMode:'custom', resultKey:'passthrough'}}];`,
    outCode: `const row=$input.first().json||{};
let data=row.data; if(typeof data==='string'){try{data=JSON.parse(data);}catch(_){data={};}}
if(row[Object.keys(row)[0]] && !data) data=row;
return [{json:{success:true,statusCode:200,data: data?.data || data}}];`,
  },
  {
...
return [{json:{success:true,statusCode:200,data:{items: Array.isArray(data)?data:[]}}}];`,
  },
  {
    ...
    method: 'POST',
    sqlCode: `const sql=\`SELECT jsonb_build_object(
  'ok', true,
...
for (const spec of specs) {
  ...
  // Replace SQL postgres query and Montar data
  const sqlNode = nodes.find((n) => n.name === 'SQL' && n.type.includes('postgres'));
  const montarData = nodes.find((n) => n.name === 'Montar data');
  if (!sqlNode || !montarData) {
    console.log('structure unexpected', nodes.map((n) => n.name));
    throw new Error('cannot clone list structure');
  }
  // Inject dynamic SQL via a Code node before SQL: reuse Restaurar request to also emit sql
  const restore = nodes.find((n) => n.name === 'Restaurar request');
  restore.parameters.jsCode = spec.sqlCode.replace(
    'return [{json:{sql',
    `const wh=$('Webhook').first().json||{}; const auth=$('Validar auth').first().json||{};
return [{json:{requestId:auth.requestId||'',userId:auth.userId,sessionId:auth.sessionId,requestStartedAtMs:Date.now(),sql`,
  );
  // Actually overwrite fully with wrapper
  restore.parameters.jsCode = `const wh=$('Webhook').first().json||{}; const auth=$('Validar auth').first().json||{};
const q=wh.query||{};
${spec.sqlCode.replace(/^const q=[\s\S]*?||\{\};/, '')}
`.replace(
    'return [{json:{sql',
    'return [{json:{requestId:auth.requestId||\'\',userId:auth.userId,sessionId:auth.sessionId,requestStartedAtMs:Date.now(),sql',
  );
  ...
  montarData.parameters.jsCode = spec.outCode;
...
// Ensure cleanup schedule exists
const cleanup = await client.query(
  `SELECT id, nodes FROM workflow_entity WHERE id='c22CacheCleanup0001' OR name ILIKE '%CACHE CLEANUP%'`,
);
let hasSchedule = false;
for (const row of cleanup.rows) {
  const nodes = typeof row.nodes === 'string' ? JSON.parse(row.nodes) : row.nodes;
  if (nodes.some((n) => n.type?.includes('schedule') || n.type?.includes('cron'))) hasSchedule = true;
}
if (!hasSchedule) {
  // create schedule workflow that calls cleanup runtime
  const schedNodes = [
    {
      id: randomUUID(),
      name: 'Schedule',
  ...
  const schedConn = {
    Schedule: { main: [[{ node: 'IA - CACHE RUNTIME cleanup', type: 'main', index: 0 }]] },
  };
  const sVid = await upsertWorkflow({
    id: 'c221CacheCleanupSched',
    name: 'SCHEDULE - AI CACHE CLEANUP',
    nodes: schedNodes,
    connections: schedConn,
    active: true,
  });
  created.schedule = sVid;
  console.log('schedule cleanup', sVid);
}
