...
return{precision,recall,documentCoverage,recallAtK,precisionAtK,mrr,hitRate,sourcePrecision:sourcePrecisionAgg,sourceRecall:sourceRecallAgg,retrievalCasesEvaluated,retrievalCasesSkipped,avgRerankLatencyMs,fallbackCount,responsePolicyWarningRate:Math.round(warnRate*10000)/10000,responsePolicyLimitationRate:Math.round(limRate*10000)/10000,responsePolicyClarificationRate:Math.round(clarRate*10000)/10000,responsePolicyAbstentionRate:Math.round(absRate*10000)/10000,responsePolicyDeclineRate:Math.round(decRate*10000)/10000,responsePolicyConflictExplanationRate:Math.round(conflictExplRate*10000)/10000,responsePolicyLowConfidenceHandlingRate:Math.round(lowConfRate*10000)/10000,avgResponsePolicyLatencyMs:avgPolLat,categoryCoverage,avgDurationMs:durations.length?Math.round((durations.reduce((a,b)=>a+b,0)/durations.length)*100)/100:null,minDurationMs:durations.length?Math.min(...durations):null,maxDurationMs:durations.length?Math.max(...durations):null,sourcesCorrectCount:results.filter(r=>r.sourcesCorrect).length,sourcesIncorrectCount:results.filter(r=>r.sourcesIncorrect).length,documentCorrectCount:results.filter(r=>r.matchedDocument).length,categoryCorrectCount:results.filter(r=>r.matchedCategory===true).length,subcategoryCorrectCount:results.filter(r=>r.matchedSubcategory===true).length,hallucinationCount:results.filter(r=>r.isHallucination).length,emptyAnswerCount:results.filter(r=>r.isEmptyAnswer).length,internalErrorCount:errors,passedCount:passed,failedCount:failed,totalCount:total,overallScore,topErrors,topDocuments,scoreFormula:'Per-case: answerQuality40 + sources30 + document20 + latency10. Run overall = average(case scores). Precision = passed/total.'}}
...
const sqlUpdateRun = "UPDATE ai_test_runs SET status = '" + finalizeStatus + "', total_cases = " + agg.totalCount + ", passed_count = " + agg.passedCount + ", failed_count = " + agg.failedCount + ", error_count = " + agg.internalErrorCount + ", overall_score = " + overallScoreSql + " WHERE id = '" + esc(runId) + "'::uuid RETURNING id;";
